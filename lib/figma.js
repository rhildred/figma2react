...
function imageURL(imgMap, sName, oNode) {
  const oImg = imgMap[oNode.imageRef];
  const sExt = oImg.contentType.split('/')[1];
  const sFileName = `public/img/${encodeURIComponent(sName).replace(/%20/g, '+')}.${sExt}`;
  const sFolder = path.dirname(sFileName);
  if(!fs.existsSync(sFolder)){
    fs.mkdirSync(sFolder, { recursive: true });
  }
  if(!fs.existsSync(sFileName)){
    fs.writeFileSync(sFileName, oImg.body);
  }
  return `url(${sFileName.split('/').slice(1).join('/')})`;
}
...
const createComponent = (component, imgMap, componentMap) => {
  ...
  const sPath = `src/components/${name}.js`;
...
`;
    fs.writeFile(sPath, componentSrc, function(err) {
  ...
  const visitNode = (imgMap, node, parent, lastVertical, indent) => {
    let content = null;
    let img = null;
    const styles = {};
    let minChildren = [];
      ...
      if (['FRAME', 'COMPONENT', 'INSTANCE'].indexOf(node.type) >= 0) {
        ...
        if (lastFill) {
          if (lastFill.type === 'SOLID') {
            ...
            styles.backgroundImage = imageURL(imgMap, node.name, lastFill);
    ...
    if (node.id !== component.id && node.type === 'FRAME') {
      print(`    <C${node.name.replace(/\W+/g, '')} {...this.props} nodeId="${node.id}" />`, indent);
      createComponent(node, imgMap, componentMap);
    } else if (imgMap[node.id] && node.type === 'VECTOR') {
      // TODO ... I broke this with the different image map
      ...
      for (const child of minChildren) {
        visitNode(imgMap, child, node, first ? null : newLastVertical, indent + '      ');
        first = false;
      }
      for (const child of centerChildren) visitNode(imgMap, child, node, null, indent + '      ');
  ...
  visitNode(imgMap, component, null, null, '  ');
  print('    );', '');
  print('  }', '');
  print('}', '');
